...
contract BountyPayment is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable abtToken;
    ...
    function createBounty(
        ...
        require(
            abtToken.transferFrom(msg.sender, address(this), amount),
            "ABT transfer failed"
        );
    ...
    function awardBounty(
        ...
        require(abtToken.transfer(winner, b.amount), "ABT transfer failed");
    ...
    function cancelBounty(uint256 bountyId) external nonReentrant {
        ...
        if (fee > 0) {
            require(abtToken.transfer(treasury, fee), "Fee transfer failed");
        }
        require(abtToken.transfer(b.poster, refund), "Refund transfer failed");
    ...
    function claimExpired(uint256 bountyId) external nonReentrant {
        ...
        require(abtToken.transfer(b.poster, b.amount), "Refund transfer failed");
    ...
    function rescueTokens(address token, uint256 amount) external onlyOwner {
        if (token == address(abtToken)) {
            uint256 excess = abtToken.balanceOf(address(this)) - totalEscrowed;
